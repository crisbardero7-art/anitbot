  ...
  try {
    switch (command) {
      ...
      case 'cum':
        ...
        break;
      case 'txs':
        if (!q) return await reply('⚠️ Debes escribir un texto. Ejemplo: .txs hola');
        try {
            // Usamos una API específica para el estilo "Brat" (letra Arial borrosa en fondo blanco)
            const textFormat = encodeURIComponent(q);
            const apiUrl = `https://api.siputzx.my.id/api/m/brat?text=${textFormat}`;
            ...
            // Convertimos la imagen recibida a sticker (WebP) usando las funciones que ya tienes
            const { imageToWebp, writeExif } = require('./lib/exif');
            const webpBuffer = await imageToWebp(Buffer.from(arrayBuf));
            const stickerBuf = await writeExif(webpBuffer, '', 'txsticker');
            
            // Enviamos el sticker
            await sock.sendMessage(remoteJid, { sticker: stickerBuf }, { quoted: msg });
        } catch (err) {
            console.error('Error en txsticker:', err);
            await reply('❌ Error al crear el sticker. Quizás la API esté caída, intenta de nuevo.');
        }
        break;
      case 'cimg':
        try {
            const { getMedia } = require('./commands/stickers');
            const media = await getMedia(msg, type);
            if (!media || media.type !== 'stickerMessage') {
                return await reply('⚠️ Debes responder a un sticker con el comando .cimg');
            }
            
            const { convertSticker } = require('./commands/tovideo');
            const result = await convertSticker(media.buffer);
            
            if (result.type === 'video') {
                await sock.sendMessage(remoteJid, { video: result.buffer, caption: '✅ Video convertido con éxito.' }, { quoted: msg });
            } else {
                await sock.sendMessage(remoteJid, { image: result.buffer, caption: '✅ Imagen convertida con éxito.' }, { quoted: msg });
            }
        } catch (err) {
            console.error('Error en cimg:', err);
            await reply('❌ Hubo un error al convertir el sticker.');
        }
        break;
      case 'dc':
