...
async function checkAdmin(sock, remoteJid, sender) {
    ...
    const normalizedSender = sender.split(':')[0].split('@')[0] + '@s.whatsapp.net';
...
async function handleGroupOptions(sock, msg, command, args, isGroup, reply) {
    ...
    // Protección para comandos administrativos
    if (['cerrar', 'abrir', 'admin', 'quitar', 'kick'].includes(command)) {
        const isAdmin = await checkAdmin(sock, remoteJid, sender);
        if (!isAdmin) return await reply('⚠️ Este comando es solo para administradores.');
    }
    ...
    try {
        switch (command) {
            ...
            case 'kick':
                ...
                if (mentionedJid.length === 0) return await reply('⚠️ Debes mencionar a alguien.');
                
                const action = command === 'admin' ? 'promote' : command === 'quitar' ? 'demote' : 'remove';
                await sock.groupParticipantsUpdate(remoteJid, mentionedJid, action);
                
                const actionText = command === 'admin' ? 'ascendido a administrador' : command === 'quitar' ? 'degradado a usuario' : 'eliminado del grupo';
                await reply(`✅ Usuario ${actionText} con éxito.`);
                break;
        }

    } catch (err) {
        console.error(`Error en comando de grupo (${command}):`, err);
        await reply('❌ Error: Asegúrate de que YO sea administrador y que tengas permisos para esta acción.');
    }
}
...
async function handleHidetag(sock, msg, args, reply) {
    const remoteJid = msg.key.remoteJid;
    ...
    const text = args.join(' ') || '📢 Mensaje Importante';
...
module.exports = { handleGroupOptions, handleSpam, handleHidetag, checkAdmin };
